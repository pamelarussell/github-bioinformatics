...
ggplot(plt_data_commits_after_pub %>% filter(variable != "Outside commit authors"), aes(variable, value_pos)) +
  geom_boxplot(aes(fill = `Commits after\npublication`)) +
  ...
  theme(legend.text = element_text(size=10),
        ...
        axis.title = element_blank(),
        strip.text = element_text(size = 10)) +
  geom_label(data = wilcox_test_commits_after_pub %>% filter(variable != "Outside commit authors"),
             aes(x = x, y = y, label = paste("p =", formatC(p, format = "e", digits = 1))),
             fill = "white",
             label.size = 0,
             color = "steelblue",
...
ggsave(file="~/Desktop/file.png", plot = plt, width = 10, height = 4)
```
...
```{r langs, fig.height = 6, fig.width = 11}
