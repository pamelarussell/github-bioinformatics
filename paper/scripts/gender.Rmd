...
gender_main %>% group_by(gender) %>% summarize(num_developers = n())
gender_high_prof %>% group_by(gender) %>% summarize(num_developers = n())
...
developers_main %>% group_by(gender) %>% summarize(num_developers = n())
developers_high_prof %>% group_by(gender) %>% summarize(num_developers = n())
```
...
paper_authors_main %>% group_by(gender) %>% summarize(num_authors = n())
paper_authors_high_prof %>% group_by(gender) %>% summarize(num_authors = n())
```
...
gender_summary_main <- gender_summary_tidy(developers_main, 
                                           ...
                                           paste("Main repos (N=", nrow(commit_author_summary_main), ")", sep = ""))
gender_summary_high_profile <- gender_summary_tidy(developers_high_prof, 
                                                   ...
                                                   paste("High profile repos (N=", nrow(commit_author_summary_high_prof), 
                                                         ")", sep = ""))
gender_summary_tidy <- rbind(gender_summary_main, gender_summary_high_profile)

# Reorder the factors
gender_summary_tidy <- within(gender_summary_tidy, stat <- factor(stat, levels=c("Developers", "Commits", "Paper authors", "First author", "Last author")))
# Add explicit zeros
  ...
  for(t2 in unique(gender_summary_tidy$stat)) {
    for(t3 in unique(gender_summary_tidy$Dataset)) {
      if(nrow(gender_summary_tidy %>% filter(Gender == t1 & stat == t2 & Dataset == t3)) == 0) {
        gender_summary_tidy <- 
          rbind(data.frame(gender_summary_tidy), data.frame(Gender = t1, n = 0, p = 0, stat = t2, Dataset = t3))
      }
    }
  }
}
...
ggplot(gender_summary_tidy %>% filter(Gender == "female"), aes(x = stat, y = p, fill = Dataset)) + 
...
team_comp_bubble_plot_data <- left_join(commit_author_summary_main %>% 
...
dev_team_size_plt_data <- commit_author_summary_main %>% 
