---
title: "Analysis of developer genders"
...
Genders were inferred for each "valid" unique name of commit authors and committers.
...
## Combine authors and committers into one table and look at gender breakdown

```{r people}
# Collapse by name and login, keeping a version with a gender call where possible
authors <- commits_no_null_login %>% 
...
## Gender breakdown of authors and committers by repo

```{r summaries}
author_summary <- commits_no_null_login %>% 
...
## Calculate Shannon index of diversity
...
## Summarize people and commits by gender
...
# Reorder the factors
summary_tidy <- within(summary_tidy, stat <- factor(stat, levels=c("Developers", "Commits")))
# Colors
...
ggplot(summary_tidy, aes(x = stat, y = p, fill = Gender)) + 
  ...
  ggtitle("Developers and commits by gender") +
...
```{r plot_buckets}
bucket_plot_data <- author_summary %>% filter(!is.na(team_type))
ggplot(bucket_plot_data, aes(x = team_type)) + 
  geom_bar(fill = "red3") +
  theme_bw() +
  ...
  ggtitle(paste("Developer team composition (", nrow(bucket_plot_data), " teams)", sep = ""))
```
...
ggplot(shannon_plt_data, aes(x = shannon_authors, y = shannon_commits, size = num_repos)) + 
  ...
  ggtitle(paste("Gender diversity of developers and commits (", sum(shannon_plt_data$num_repos), " teams)", sep = "")) +
...
ggplot(team_size_plt_data, aes(x = team_size, y = shannon, size = num_repos)) + 
  ...
  ggtitle(paste("Gender diversity and team size (", sum(team_size_plt_data$num_repos), " teams)", sep = "")) +
  theme_bw() +
  theme(plot.title = element_text(size = 18),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
```{r push_to_bq}
upload_job_authors <- insert_upload_job(project = proj, 
                                        dataset = ds_analysis, 
                                        table = table_gender_authors, 
                                        values = author_summary,
