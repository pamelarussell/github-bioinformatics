...
add_file_counts_bool("reflective")
...
# Amount of code in test cases for interpreted vs compiled languages
test_cases_by_exec_method <- as.tbl(test_cases_no_data) %>% 
  mutate(language = tolower(language)) %>% 
  left_join(lang_exec_method, by = "language")

join_tbl(
  test_cases_by_exec_method %>% 
    filter(interpreted) %>% 
    group_by(repo_name) %>% 
    summarize(total_bytes_test_cases_interpreted = sum(size))
)
repo_data$total_bytes_test_cases_interpreted <- sapply(repo_data$total_bytes_test_cases_interpreted, na_to_zero)
repo_data <- repo_data %>% 
  mutate(pct_bytes_in_test_cases_interpreted_no_data = 
           total_bytes_test_cases_interpreted / total_bytes_no_data_interpreted) 

join_tbl(
  test_cases_by_exec_method %>% 
    filter(compiled) %>% 
    group_by(repo_name) %>% 
    summarize(total_bytes_test_cases_compiled = sum(size))
)
repo_data$total_bytes_test_cases_compiled <- sapply(repo_data$total_bytes_test_cases_compiled, na_to_zero)
repo_data <- repo_data %>% 
  mutate(pct_bytes_in_test_cases_compiled_no_data = 
           total_bytes_test_cases_compiled / total_bytes_no_data_compiled) 
