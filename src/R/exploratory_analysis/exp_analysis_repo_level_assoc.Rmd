...
add_file_counts_bool <- function(col) {
  ...
  # Total file size
  bytes <- filtered %>% group_by(repo_name) %>% summarise(xxxxx = sum(size))
  colname_bytes <- paste("total_bytes_", col, sep = "")
  colnames(bytes) <- c("repo_name", colname_bytes)
  join_tbl(bytes)
  repo_data[[colname_bytes]] <<- sapply(repo_data[[colname_bytes]], na_to_zero)

  # Total number of files
  files <- filtered %>% group_by(repo_name) %>% summarise(xxxxx = n())
  colname_files <- paste("total_files_", col, sep = "")
  colnames(files) <- c("repo_name", colname_files)
  join_tbl(files)
