...
plt_data_outside_contrib <- as.tbl(repo_data_all) %>% 
  select(forks_count, subscribers_count, watchers_count, num_non_committing_authors) %>% 
  dplyr::rename(
    `Total forks` = forks_count, 
    `Total subscribers` = subscribers_count, 
    `Total watchers` = watchers_count,
  ...
  melt(id.vars = "Outside commit authors") %>%
  group_by(`Outside commit authors`, variable, value) %>%
  summarize(`Num repos` = n())

ggplot(plt_data_outside_contrib) +
  geom_point(aes(size = `Num repos`, x = `Outside commit authors`, y = value)) +
  theme_bw() +
  theme(legend.text = element_text(size=12),
        legend.title = element_text(size = 14),
        axis.text = element_text(size = 12),
...
plt_data_commit_authors <- as.tbl(repo_data_all) %>% 
  ...
  group_by(`Total forks`, `Total subscribers`, `Total watchers`, `Commit authors`, `High profile`) %>%
  summarize(`Num repos` = n()) %>%
...
```{r commits, fig.width = 7, fig.height = 3}
plt_data_commit_authors <- as.tbl(repo_data_all) %>% 
  select(commits, mean_commits_per_month, consecutive_months_with_commits, is_high_profile) %>% 
  dplyr::rename(
    `Total commits` = commits,
  ...
  group_by(`Total commits`, `Mean commits/month`, `Max cons. months with commits`, `High profile`) %>%
  summarize(`Num repos` = n()) %>%
