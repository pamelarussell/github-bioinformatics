...
class CommentExtractorJava(CommentExtractor):
    """ Comment extractor for Java code """

    def process_single_line_match(self, match):
    ...
    def process_multi_line_match(self, match):
    ...
    def extract_comments(self, file_contents):
        ...
        single_line_comments = [self.process_single_line_match(match) for match in iter_single]
        
        multi_line_re = r'/\*.*?\*/'    
        iter_multi = re.finditer(multi_line_re, file_contents, DOTALL)
        multi_line_comments = [self.process_multi_line_match(match) for match in iter_multi]
